...
<template>
  <div class="question-container p-6 border border-gray-200 rounded-lg">
    <div class="question mb-6">
      <h3 class="md:text-xl text-md font-semibold mb-2">{{ question.question }}</h3>
    </div>

    <div class="options space-y-3">
      <div v-for="(option, index) in question.options" :key="index"
        class="option p-4 border rounded-md cursor-pointer transition-all text-xs md:text-sm" :class="{
    ...
    <div class="navigation flex justify-between mt-8 text-xs md:text-sm">
