...
const performance = computed(() => {
  if (percentage.value >= 90) return { message: dict.excellent, emoji: '🏆' };
  if (percentage.value >= 70) return { message: dict.great_job, emoji: '🎉' };
  if (percentage.value >= 50) return { message: dict.good_effort, emoji: '👍' };
  return { message: dict.keep_practicing, emoji: '💪' };
});
...
<template>
  <div class="results-container text-center">
    <h1 class="text-4xl font-bold text-gray-800 mb-5">{{ dict.quiz_result }}</h1>
    ...
    <p class="text-xl mb-6">
      {{ t('you_scored', { score, total, type: quizTypeName }) }}
    </p>
    ...
    <div v-if="wrongAnswers.length" class="wrong-answers-section mt-8 mb-8 text-left">
      <h3 class="text-2xl font-bold text-gray-700 mb-4">{{ dict.review_incorrect }}</h3>
